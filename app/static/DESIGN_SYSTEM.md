...
| 2026-08-24 | **Change role** (first user test, plan D2.4): a `text-act` on every reference card that reopens the role dialog prefilled with the reference's current family, title, and both jurisdiction lists. A tester filed a spaceship as `CHARACTER_LIKENESS` because that was the role the walkthrough named, and nothing in the app could take it back — the PATCH endpoint existed and no screen had ever called it, so the role picked at intake was permanent. Re-scoping keeps the approval and the image; only what the reference GOVERNS changes, and it is journaled | Reference library, every card | Built from canon — the existing role dialog, no new pattern. Review (1) whether `Change role` belongs beside Crop/Reject or under a quieter affordance, since it is rare and consequential; (2) whether the jurisdiction fields should be prefilled from the OLD family (as now, so nothing is silently lost) or reseeded from the new family's suggestions; (3) whether the card should show that a reference has been re-scoped |
