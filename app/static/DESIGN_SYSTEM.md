...
| 2026-08-23 | **An answer you can save** (first user test, retest): the interview answer field had no save affordance — Enter or lose it — so typing and clicking away discarded the answer with no request and therefore no feedback. It now carries `.q-edit`: the input, a `primary` Save, a `text-act` Cancel and a Courier `ENTER SAVES · ESC DISCARDS` hint, all on one line beside the field. Clicking away SAVES; Escape is the only discard. The stated gate from the row below also grew from dim caption text into `.repair-gate`, a bordered full-ink chip — it was stated and still missed among eight toolbar controls | Stage 02 open questions; Repair Region dialog | Built from canon — `primary`/`text-act`/`mono`, no new tokens. Review (1) whether blur-saves is the right default for this app, since canon has no ruling on implicit commit; (2) whether a four-element edit row belongs inline or should displace the question text; (3) the chip's border weight against the disabled primary next to it |
