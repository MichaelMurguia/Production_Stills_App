...
| 2026-09-12 | **The roster counts scenes, and casting is one gesture** (CAST_CHARACTER_SCREEN §1): a card's third line is `n SCENES` rather than a photograph tally — what the draft says about a subject, not what the app holds. An uncast chip and the manual `+ Cast` row now CAST and land on the empty detail screen, where the modal used to ask again for a name, a role line and a profile the read already supplied. The ribbon's tiles still open the modal (user, 2026-09-10). The plan's 12px floor is applied across both screens, including `#cast-screen`-scoped overrides for `.wiz-group-label`, `.cost` and `.text-act`, which are app-wide at 9.5–10.5px | The cast roster and detail | Built from canon. Review (1) two casting doors now reach two components — a ribbon tile opens the modal, a roster chip casts outright; (2) the scoped type overrides are a patch over an app-wide 9.5px drift that still wants a deliberate pass |
