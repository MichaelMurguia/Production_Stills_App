...
|---|---|---|---|
| 2026-08-16 | **A migration surface** (user: "I still have 2 CANYON_GRM breakdowns. Lets consolodate."): revisions are retired, so a unit that still exists as `_R2` gets a grey lock-strip above the breakdown table naming the split (`CANYON_GRM` exists as 2 breakdowns — `CANYON_GRM`, `CANYON_GRM_R2`) and one verb, `Consolidate into one`. The confirm states the whole consequence — which document survives, that every take folds into one pool keeping its image and its approval snapshot, that the older documents are archived inside the survivor, and that nothing is deleted. The strip renders nothing once every unit is single, which is its intended resting state | Stage 03, above the breakdown table | Built from canon (lock strip + block-act + askConfirm) — review two things: (1) grey rather than amber, chosen because a list can hold several split units and several ambers is no signal at all, though this IS the one act worth doing on that page while it shows; (2) whether a control that deletes itself once used wants any distinct treatment, or is right to look exactly like every other stated gate |
...
- **2026-08-16** — **Revisions collapse into one breakdown** (user
  ruling "Yes Collapse", asked for concretely). `revisions.consolidate()`
  folds a `_R<n>` chain into the base id: the newest document becomes the
  breakdown, older ones are archived inside it, every take moves into one
  pool and is retagged with `consolidated_from`, and a copy of every file
  is written to `data/consolidations/` first. Nothing is deleted. The UI
  is a lock strip above the breakdown table that exists only while a unit
  is still split.
