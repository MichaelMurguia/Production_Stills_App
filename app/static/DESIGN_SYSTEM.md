...
| 2026-08-22 | **The rendering picker becomes PARTLY photographed** (user supplied five renders). Five of the ten styles — Production Painting, Photo Real, Ink & Wash, Hand-Drawn Cartoon, 3D Rendered Cartoon — now show a real frame; the other five keep their drawn diagram, and `PLATES ARE DIAGRAMS UNTIL THE REFERENCE LIBRARY LANDS` stays because it keys off the CARDS, not the library. One frame per style rather than cinematography's and world texture's three, so the photographed cell is a single narrow thumb beside a full-width diagram | Stage 02, Board Rendering picker | Built from canon — the manifest and the rich card already carried this. Review (1) a grid where a one-frame card sits beside a three-frame card and a diagram card, which is three shapes in one row; (2) whether a single frame should fill the cell the way a diagram does, since B3 forbids padding to three but says nothing about widening one |
