...
|---|---|---|---|
| 2026-08-16 | **A required object can overrule its reference match** (user: "its green but the reference is wrong so I want to delete it"): `Not this object's reference` in the plate viewer unpairs the object from the matched group(s) — the tile returns to `+ REF`, the plates stay in the library and stay attached to every other object that really names them. Stored on the PANEL (`ref_exclusions`), journaled, lock re-stamped, refused on an approved take. Choosing that same group deliberately through `+ REF` undoes it | Panels workbench, step 02 plate viewer | Review (1) the verb's wording, which has to read as UNPAIR and not DELETE or nobody will risk it; (2) that a false green is only correctable from INSIDE the viewer, two clicks from the tile that is wrong; (3) whether the exclusion should be visible anywhere as a standing rule, since today it is invisible once made |
