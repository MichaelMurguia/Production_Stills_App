...
| 2026-08-01 | DUPLICATION FLAGGED BY THE USER — needs a design fix: wizard step 5 (Draft & review → "Draft Art Direction Bible" + its #wiz-bible review area) sits directly above the Art Direction Bible editor panel ("Save Art Direction Bible") — two adjacent amber fills and two surfaces claiming to be the bible. They should merge into one draft-review-save flow. Also: the editor's "showing built-in default — save to make it yours" hint is stale since the template default was deleted (the editor now starts empty) | Wizard step 5 + bible editor panel | The draft target and the living document grew separately; designer should rule on the single merged surface |
