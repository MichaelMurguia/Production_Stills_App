...
| 2026-08-13 | **Evidence ledger row is selectable** (user-directed): panel id and object are `<select>`s — the object offered from the chosen panel's required objects that lack a row (stored values always round-trip); the citation input gains reference-library type-ahead via a shared `<datalist>` (`REF-0040 — VEHICLE_GEOMETRY — …`) | Breakdown sheet editor, evidence ledger | Built from canon controls — review the empty-state option copy ("— every required object has a row —") and the datalist entry format |
