...
|---|---|---|---|
| 2026-08-16 | **One question per anchor** (user: "we now have duplicative entries and we should consolidate"). The look interview and the style anchors were the same four questions asked in two media — `Medium & finish` and Board Rendering's SETS line were the same three words, and `Palette & light` was one box answering two different anchors. The words are folded INTO the anchor card they duplicated: every card now carries `Add images` AND an `IN WORDS` half, and step 01 keeps only what no anchor can hold — touchstones, the never-list, notes. An anchor is answered by a picture OR by words, so both badges count either, and a card with only words reads `IN WORDS` rather than `NONE` over its own answer | Stage 02, steps 01 and 02 | Ruling wanted on (1) the card as a two-media object — one question, a picture half and a words half, which is a shape nothing else in the app has; (2) whether `IN WORDS` belongs in the count badge or wants its own mark, since a number and a phrase now share one slot; (3) step 01's new subtitle `WHAT NO ANCHOR CAN HOLD`, which explains a step by what it is NOT |
