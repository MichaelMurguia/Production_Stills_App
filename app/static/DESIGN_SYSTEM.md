...
| 2026-08-31 | **The read walk is 10s and uneven; its route is permanent** (user-directed): the scene ticker spread a finished local parse over 60s at an identical interval per scene ("each scene is the exact same length of time") — now 10s with ±45% jitter per row, clamped 160–700ms. The finish panel showed a count and a Prod. Design button for 7s then removed itself, putting a primary action on a timer; it now shows nothing and goes, and the route is a permanent full-width `.ds-go` ghost button in the stage-01 side rail under the counts the read produced | Stage 01, during and after a read | Built from canon — the button is `.ghost`, since stage 01's one primary act is uploading a draft. Review (1) the walk is honest content at a chosen pace — whether it should exist at all; (2) the route's placement under DOWNSTREAM OF THIS FILE |
