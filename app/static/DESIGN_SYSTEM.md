...
**The rule is measured, not listed** (DELTA, 2026-09-12b). The first pass
replaced a fixed list of greys and a verifier found the ones the list
missed. So: every text colour must MEASURE ≥ 7:1 against the surface it
sits on, and anything that fails is replaced by the nearest token above.
`tests/test_design_tokens.py::TheColourFloorIsMeasuredNotListed` computes
it. Only glyph colour is governed — `border-color`,
`text-decoration-color`, `outline-color`, backgrounds, scrims and swatch
values are explicitly out of scope, and a swatch's fill is the
production's own data rather than anything this system owns.

**Status colour lives on the rule, not on the glyph.** `--hold` is not
one of the seven and colours no text at all; `--bad` is text only at
headline scale or beside a hairline. A held or failed thing states itself
with a 2px left rule (or its border) and reads its words in `--ink-faint`
— the `.cd-blocked` idiom, applied everywhere. Before this, a stated
blocker was the most important sentence on its card and the least
legible one.

**`--panel2` is the one ground where the palette falls short.** Measured,
`--ink-faint` is 8.33:1 on `--ground` and 6.56:1 on `--panel2`; `--accent`
is 8.83 and 6.96. Every other surface clears. Fixing it means changing an
ink the floor fixes or darkening a surface token it does not mention —
a designer's call. Recorded as a number in the token tests so it cannot
...
| px | face | used for |
|---|---|---|
| 15 | Courier, **uppercase only** | section kickers, step labels — tracked ≥ .12em |
| 17 | Courier, **uppercase only** | state marks, counts, badges, cost notes, legends, turn ids |
| 18 | Archivo | tertiary links, secondary prose |
| 20 | Archivo / Courier | body prose, buttons, captions, list items, form fields; sluglines, file paths, page refs |
| 22 | Archivo | step titles, modal titles, detail-screen description |
| 24 | Archivo | bible section subjects |
| 27 | Archivo | bible `##` headings |
| 30 | Archivo | the logline |
| 33 | Archivo | screen titles |
| 36 | Courier | step numbers |
| 48 | Archivo | the bible's production title |
...
### The type scale is ONE table (2026-09-13)

**Every font-size in the app is one of eleven role tokens, declared at the
top of `styles.css` under `THE TYPE SCALE`. There are no literal px sizes
anywhere else — not in the stylesheet, not in `app.js`, not inline.**

| token | base | role |
|---|---|---|
| `--t-kicker` | 15 | Courier CAPS — section kickers, step labels |
| `--t-mark` | 17 | Courier CAPS — states, counts, badges, cost notes |
| `--t-minor` | 18 | Archivo — tertiary links, secondary prose |
| `--t-body` | 20 | Archivo body; Courier slugs, paths, refs |
| `--t-title` | 22 | step titles, modal titles, detail description |
| `--t-subject` | 24 | bible section subjects |
| `--t-head` | 27 | bible `##` headings |
| `--t-logline` | 30 | the logline |
| `--t-screen` | 33 | screen titles |
| `--t-step` | 36 | Courier step numbers |
| `--t-prod` | 48 | the bible's production title |

Each is `calc(<base>px * var(--t-scale))`. **Resizing the whole app is one
line**: `--t-scale`. Resizing one role is one line. The store has the same
table plus `--t-hero` for its sales headline.

Why it exists: four plans and two user rulings moved this scale in a single
day, and each move meant rewriting 300–550 literal declarations and then
chasing every test that pinned a number. That is an architecture fault, not
bad luck (user, 2026-09-13: *"You should have a table of strings and font
sizes and be able to change like 5 lines in settings"*). Tests pin the
ROLE now, never the number, so a scale change touches nothing but the
table. `tests/test_design_tokens.py::TheTypeScaleIsOneTable` fails the
build if a literal size reappears.

**`--t-scale` is currently `.8`** — 20% below the ruled ladder, by the
user's instruction on 2026-09-13. At `1` the smallest rendered glyph is
10px; at `.8` it is 7px. The ladder's ratios are unchanged.

### The ratios: TYPE_SCALE_R2 × 1.5 (2026-09-12)

`TYPE_SCALE_R2_2026-09-12.md` supersedes the floor's own §1 table and
restores range — its complaint, correctly, was that a flat floor made
"kickers, captions and body all read at nearly one size". Its ladder is
10/11/12/13/15/16/18/20/22/24/32 in FONT-SIZE, and its floor tier renders
7px caps and 5px x-height on this platform, which is below the ruling
below. **Ruled by the user: keep the ink floor, keep R2's ratios —
multiply the ladder by 1.5 so its floor tier lands at 10px of ink.**

Two tiers carry a condition. 15px and 17px Courier clear 10px on CAP
height and fail it on x-height (7px and 8px), so they are legal **only
where the text renders uppercase** — which is R2's own rule for its
equivalent tier. A rule earns one by declaring the transform or by being
MEASURED rendering uppercase; anything unproven stays at 20px, where both
cases clear. The tight tiers grow as surfaces are measured, never by
assumption.

### The 10px glyph floor (user ruling, 2026-09-12)

**No letter, upper or lower case, renders smaller than 10px. Ever.**

That is a measurement of INK, not of `font-size`, and the two do not
track each other — not across faces, and not between cases of the same
face. Measured by pixel scan:

| face | size | cap | x-height |
|---|---|---|---|
| Archivo | 14px | 10 | 8 |
| Archivo | **18px** | 13 | **10** |
| Consolas | 15px | 10 | 7 |
| Consolas | **20px** | 13 | **10** |
| Courier New | 15px | 8 | 6 |

So the live scale is **Archivo 18 / Courier 20**, with 19, 20, 24 and 34
above them. Everything below 18px is retired: at those sizes a lowercase
letter is 6–9px of ink whatever the font-size says.

Courier is 20px rather than 15 even though most of it is uppercase,
where 15px already clears the cap floor. One size, no exceptions: the
per-rule version of this needed a list of which classes render lowercase,
and that list was wrong three times in a row because CSS cannot select on
case and a more specific rule kept winning. A floor with exceptions is a
floor someone has to maintain.

**The Courier step is 15px, not 13** (user-caught 2026-09-12: "site still
has 9px tall fonts"). The floor is written in font-size; what a person
reads is CAP HEIGHT, and the two do not track each other across faces.
Measured by pixel scan:

| face | cap@13 | cap@14 | cap@15 | cap@17 |
|---|---|---|---|---|
| Courier New | 8 | 8 | 8 | 10 |
| Consolas | 8 | 9 | 10 | — |
| Cascadia Mono | 9 | 10 | 10 | — |
| Archivo | 9 | **10** | 11 | — |

Courier New is hinted flat: its cap height is 8px from 11.5px through
15px, so raising the floor from 11.5 to 13 moved the rendered letter by
**zero** and the fault the floor names survived it. Two levers reach cap
height — the size and the face — and both moved the minimum needed:
`--mono` now leads with Consolas (narrower than Courier New, so no label
costs more room), and Courier sits at 15px, where it renders the same
10px cap Archivo gives at the 14px prose floor. **Every Courier rule
carries a size**; one that inherits the 14px body renders 9px caps, so
the shared family group sets 15px early and components override it later.
...
headings (600, `--ink`) and body (400, `--ink-dim`) · `15px` Courier for
everything else. Two roles may share 15px separated by weight ONLY because 24px
...
**A state shared by every row belongs to the TABLE, not to the rows.**
`NEEDS THE BIBLE` was on all seventeen location rows — one fact, restated
per line, beside a SHEET column already reading `NONE` on each. It is one
sentence above the table now, with the route to resolve it. The test is
the same as the others: if the reader has already been told, do not tell
them again.

**Rule 2 outranks a habit.** An all-caps Courier run is not automatically
machine data. `TURNS ON — <a 23-word narrative sentence>` was uppercased
into Courier because everything around it was; it is prose and now reads
as prose, in Archivo and its own case. The KICKER stays Courier — a label
is machine data even when the thing it labels is not.
...
submit reads `Scan Screenplay` / `Break down the pasted section` (and, before
the doors split, `Create an empty sheet`).
...
> **State of this queue, 2026-08-22 — THIRTEEN rows against a threshold of
> about four.** It has not been reviewed since the pass that produced
> RULE_PASS_2 and TRIAGE. Three weeks of work sit here, and it is now large
> enough that reviewing it row by row is the wrong shape — several rows are
> the same question asked in different places.
>
> **The threads worth ruling on as threads, not rows:**
>
> 1. **Long operations that must be watched.** Three rows describe the same
>    surface: the screenplay read, its preview, and the bible/palette act.
>    One pattern (`.rd-*`) now serves all of them and is still uncanonized.
>    Rule on the pattern and three rows collapse.
> 2. **Gates.** Four rows are gates — the screenplay lock, the palette lock,
>    the tour's held step, the chain link. Canon has `.up-gate` (a faint line
>    under a disabled control) and now `.pal-gate` (a framed block that
>    out-weighs the control). One scale with two weights, or two patterns?
> 3. **Verdicts on a SET.** `.sw-collide` reports that a group of items
>    repeats itself, where every item is individually fine. Canon has no
>    vocabulary for this and it will recur — the same shape fits a board
>    whose panels are too alike.
> 4. **Controls whose verb changes.** The bible's one button and the panel's
>    grammar select both replace a choice-between-controls with a control
>    that knows the state. Is that canon now?
>
> The rest are placement and copy questions and can be ruled quickly.

| Date | Pattern | Used in | Why nothing existing worked |
|---|---|---|---|
| 2026-08-22 | **The breakdown intake has TWO doors again** (user: "I want two separate columns") — **this REVERSES C1 / RULE_PASS_2 C, ruled 2026-08-18**, which merged them, and it is written here so the next pass does not re-merge them from the same reasoning. That ruling gave three reasons and all three are answered, not ignored: it said the blank door was *"a strict superset"* calling the same endpoint — so the blank door now carries NO brief, NO paste and NO panel list, makes an empty sheet and nothing else, and the two call different endpoints (`/api/specs` vs `/api/specs/autofill`), which is what makes them two acts rather than one form with a hidden branch; it said *"amber sat on the superseded one"* — amber is on the AUTO door and the blank sheet's submit is a ghost; it said *"at 1440 the grid stacked"* — `.intake-doors` stacks at 980, verified side by side in a 1420 capture. The auto door keeps its own two acts behind one recomputing verb (Scan Screenplay / break down the pasted section); the third act it used to hide is the other door now **Rebuilt the same day from the user's own mock**: label-left `.door-row` rows so both doors sit at a readable height; **`What should I get?` IS the search** — every typed word must appear somewhere in a heading or its location, in any order, so `terra` returns all seven Terra Nova locations with their scene counts and `terra bridge` returns the one; locations lead, and picking either writes the brief the scene anchor matches on. A native `<datalist>` was tried first and withdrawn: it filters however the browser feels like it, cannot rank, and cannot show that a location holds seven scenes. **The search runs while you type, and the pick is a POINTER** (ruled same day, after a submit-only version lived for one afternoon): the search was moved onto the submit because results under the caret interrupted writing a brief — the user's reply was that it no longer auto-filled, so the suggestions are back on `input` and `focus`, and what the button does now is READ a saved scene rather than run a search. Picking a scene saves `{heading, line}`; the field keeps only the TITLE and the line beneath it states what was saved — `SCENE SAVED · LINE 2857`, Courier, `--ok` (a met condition, not an act, so not amber). **Everything else the pick knows moves into the paste row's slot** (user, 2026-08-22: "whatever lands in that What should I get field is awkward… all the other information, put it in a section below the select field and have it take the place of the Or paste a section area"): a `The scene` / `The location` row carrying `READ EXACTLY — LINE 2857` or `MATCHED BY NAME — 7 SCENES`, the composed brief as an EDITABLE textarea — it is the ask, and the composed sentence is a starting point — and a `Clear selection` text-act where `Open screenplay ↗` sits in the row it replaced. Removing the selection brings the paste row back. **The results list is also the door's source switch**: `None` and `Paste a section` stand above the matches in `--ink-dim`, because with a selection made the paste row is off screen and a switch you cannot reach is not one. Paste mode is STATED (`pasting`), not inferred from the box having text — the switch turns it on while the box is still empty; emptying the box is the one way back. The submit is `Scan Screenplay`, and it quotes the scene AT that line instead of matching the brief to a slugline — the match has landed on the wrong scene twice, and a location holding seven scenes cannot be narrowed by matching at all. Typing over the TITLE drops the pointer and the brief with it — typing means searching again, and an orphaned hand-edited brief would be invisible in a hidden row and still submitted. Editing the brief itself does not: they are two fields now precisely so the ask can change without the reference. A replaced draft makes the pointer refuse rather than read the wrong scene. A no-match says nothing at all — two versions of a message here were tried and both were noise over a field the user was still typing in. **The brief and the pasted section are alternatives** — typing in one makes the other unavailable, because `source_text` REPLACES the screenplay and a brief beside it would name material the model has been told not to look at. A panel-count dropdown lived here for an afternoon and was withdrawn when the mock kept the list | Stage 03, New breakdown | This is the 2026-08-06 **Two doors, one section** canon restored to the surface that was its own worked example — that rule is on line 254 and still cites Breakdowns, so canon and code agree again. Review (1) whether the reversal should be folded back into canon as-is or whether the merge's reasoning applies to some OTHER pair; (2) the blank door's short column beside a tall one, which the merge did not have to solve; (3) `SCENE SAVED · LINE 2857` — a field that reports a saved reference in Courier beneath itself, which canon has no vocabulary for, and which disappears the moment the field is typed over; (4) ONE SLOT holding either of two `.door-row`s depending on state, which is new — canon disables an alternative (`.is-off`), it has never swapped one out; (5) two action rows at the head of a results list, which makes a list of matches also a mode switch |
| 2026-08-22 | **The style pickers became document-backed, and rich** (user: "are there in depth prompts for each?"): world texture (5) and rendering style (9) were arrays in `app.js` carrying a one-line description and a ~60-character directive; they are now read from `docs/WORLD_TEXTURE_STYLES.md` and `docs/RENDERING_STYLES.md` in the same shape cinematography has used since 2026-08-16. The pickers upgraded themselves as a consequence: both now render the RICH card — full description, `Read the grammar` reader, an Avoid fence — where they previously rendered the plain card with a one-line `desc` and a `NOT` line | Stage 02, World Texture and Rendering Style pickers | Built from canon — the rich card already existed for cinematography. Review (1) whether the plain card still has any user at all, or whether it should be retired now that all three anchors are rich; (2) the two-up rich grid at the anchor column's width, which was designed for eight cinematography grammars and now also carries nine rendering styles |
| 2026-08-22 | **Picker plates are photographs, not diagrams** (user supplied the renders): cinematography (8 grammars) and world texture (5) now show THREE real frames per style — the same three subjects under every condition, so a set reads as one comparison rather than five unrelated pictures. Rendering style still shows its drawn `.rs-plate` diagrams, and a rich card falls back to its diagram rather than the `REFERENCE FRAMES — NOT YET IN THE LIBRARY` placeholder, which now appears only where a diagram is genuinely being shown | Stage 02, all three style pickers | Built from canon — `.rs-frames` already existed. Review (1) the mixed state, where two anchors show photographs and one shows diagrams in the same visual language; (2) `.rs-cards-rich .rs-frame` at 16/9, which makes a drawn plate take a photograph's shape so the two read as the same kind of thing — arguably they should not |
| 2026-08-22 | **DESCRIBE THE LOOK** (user: "the breakdown sheet is broken because — what is style reference?"): the gate chain's stage 02 link read `ADD STYLE REFERENCE` with a sub-line naming "board layout, cinematography and rendering plates — the three anchors", which named an anchor that is not one of these and miscounted the four as three. It now reads `DESCRIBE THE LOOK` — "any one of the four anchors — world texture, colour, cinematography, rendering style — in words or a picture" — and is satisfied by an anchor stated in EITHER form, where it previously counted approved images only | Status gate chain, stage 03's locked state | Built from canon — a gate-chain link. Review the copy only: whether a chain link should name a VERB (`Describe the look`) or a CONDITION, since the others in the chain are a mix of both |
| 2026-08-22 | **The bible is written and coloured in one act, watched** (user: "generating the art direction bible and swatches should happen in a single step ... same razzle-dazzle feedback as the screenplay read"): one press writes the bible, saves it, and proposes a palette per design language. A `.rd-*` progress panel — the read surface's own vocabulary, reused not reinvented — carries four phases: **ASSEMBLE** (local, real: one ladder row per design language, the director's answers beside it), **WRITE** (one call, `.busy` spinner, states it reports nothing until done), **SECTIONS** (the returned bible's own headings, and each language's row says `OWNS ITS COLOUR` or `NO COLOUR OF ITS OWN`), **COLOUR** (one call, then each row's cell becomes that language's real ramp). Plus `.sw-collide`, a **dismissible** `--bad-line` block reporting a measured verdict on the SET — 23 of 27 colours repeating across four languages | Stage 02, step 4 | Reuses the read panel wholesale; `.bb-ramp` and `.sw-collide` are new. Review (1) whether one pattern serving two long operations should be named canon now rather than sitting uncanonized twice; (2) `.sw-collide` — a verdict on a set, which canon has no vocabulary for: every item can be defensible while the set repeats, and it reports without blocking so it must not wear amber; (3) the ladder row whose right-hand cell changes meaning three times as the phases advance |
| 2026-08-22 | **One button, four verbs** (user: "I should not have to detail out how creating, editing and saving should work — make it all one button"; then "it should be at the top of the section — have a regenerate button only once created"): stage 02's Bible panel now carries ONE primary control at the top whose verb is a function of state — `Create Art Direction Bible` when empty, `Save Art Direction Bible` when text is pasted but unsaved, `Edit` when saved, `Save` while editing — plus `Regenerate`, which appears only once a bible exists and hides again while editing. Escape discards an edit; saving an unchanged bible re-locks without bumping the revision. Previously: stage 02's Bible panel had a gold `Draft Art Direction Bible` that drafted into an editor and stopped, and a grey `Save Art Direction Bible` underneath that was the only thing which actually opened Breakdowns. The gold verb is now `Create Art Direction Bible` and it saves what it writes. The grey button reads `Edit` over a read-only editor (`--field` ground, `--ink-dim` ink — not dimmed, which would say unavailable), becomes an amber `Save Art Direction Bible` while editing, and returns to `Edit` on save | Stage 02, step 4 | Built from canon — two existing buttons, one new read-only state on a textarea. Review (1) a primary verb that both generates and commits, which no other step does — everywhere else generation proposes and a second act accepts; (2) whether the grey button changing its own label is legible, or whether Edit and Save should be two controls with one disabled; (3) the read-only textarea's ground, which is the first place this system has used `--field` to mean *not being edited* rather than *input* |
| 2026-08-22 | **A panel can name or refuse the cinematography grammar** (user): the grammar was production-wide with no way to vary or suppress it per render, and the Production Design card carried a `RENDERS` on/off strip that has now been **retired** (user: "there should be no switch on the panels in the Production Design tab — that sets the default, it should be over-rideable on the panel render tab"). Two levers, not three. The camera row gains a `Grammar` select carrying the same three states every camera axis already uses — `— production default —`, `None — no grammar`, or one of the eight — saved through the existing camera amend, so it is journaled, re-stamps the lock and is refused once a take is approved. It appears only where a blank option exists, so the production-defaults card does not offer a control pointing at itself. The take's stamp records which grammar rode and whether it came from the panel or the production, and a panel that refused records `refused: true` | Stage 03 sheet editor, stage 04 workbench | Built from canon — the select is a sixth `.cam-field` in an existing row. Review (1) whether a grammar belongs in a row labelled CAMERA at all, since it is not a camera axis and the row's heading now undersells it; (2) `None — no grammar` sitting above the named grammars rather than at the foot of the list; (3) whether a panel that overrides the production default should say so on the card face, where today it is only visible inside the opened camera editor |
| 2026-08-23 | **The open questions state that they are optional** (user: highlight "Optional. You can do this over time"). The section header gains `OPTIONAL — YOU CAN DO THIS OVER TIME` in `--ok` green Courier beside the count. Green, never amber: amber marks what blocks, and this line exists to say the questions do NOT | Stage 02, Scene Scan → Open Questions header | New use of `--ok` for reassurance copy rather than a met condition. Review whether that stretches the token — the alternative was plain `--ink`, which did not read as highlighted |
| 2026-08-23 | **A design language's swatches approve as ONE palette** (user: "the swatches need to be approved as one... only one approve button"; "it should say 4 proposed, not 24"). The proposal viewer's rows keep per-colour `Edit` (rename + repaint + value-key pair) and per-colour `Reject` (drop one colour, the rest approve together), but the VERDICT moved to the footer: one `Approve palette` primary per group, `Approve all N palettes` across groups. The footnote `EACH VERDICT IS ITS OWN REFERENCE RECORD` became `A LANGUAGE'S SWATCHES APPROVE AS ONE PALETTE`. Every proposal row in the anchor column carries its own `Edit` / `Approve` text-acts — the ramp states every colour on the row, so a group verdict judges nothing unseen. The anchor badge counts GROUPS: a column of pending proposals reads `4 PROPOSED` (PROVISIONAL), an approved column reads the palette count, never the colour count; `Review all` names palettes too. Render-time was already one reference — `palette_plate.collapse` folds a language's swatches into one composite before the 14-cap — so the records stay per-swatch underneath and only the surfaces changed | Stage 02 Color Palette column, swatch viewer, Reference ramps | Amends the D8/R5 vocabulary: per-swatch verdicts are retired, the unopened-gate on Approve-all keeps its rule but the per-row Approve bypasses it deliberately (the row shows all its colours). Review (1) a `primary` inside a modal footer that also carries text-acts; (2) `ok-act` green on the row's Approve beside a plain Edit |
| 2026-08-22 | **The rendering picker becomes PARTLY photographed** (user supplied five renders). Five of the ten styles — Production Painting, Photo Real, Ink & Wash, Hand-Drawn Cartoon, 3D Rendered Cartoon — now show a real frame; the other five keep their drawn diagram, and `PLATES ARE DIAGRAMS UNTIL THE REFERENCE LIBRARY LANDS` stays because it keys off the CARDS, not the library. One frame per style rather than cinematography's and world texture's three. **RULED the same day, by the user: a row is as wide as it has frames** — `.rs-frames[data-n]` gives one frame the full card width and two frames a half each, so a lone render reads at the diagram's size instead of as a thumbnail beside it. B3 is untouched: nothing pads to three, the row stops pretending there are three. **Also ruled: the captured house plate opens like every other frame** — `.rs-shot` carried no `data-lb`, which is what the picker binds the lightbox to, so the production's OWN plate was the one picture in the modal you could not open. The drawn diagram still gets none: an SVG at 68×56 has no full size. Every openable frame now says so with `cursor: zoom-in` | Stage 02, Board Rendering picker | Built from canon. Review (1) a grid where a one-frame card sits beside a three-frame card and a diagram card, which is three shapes in one row — the widths agree now but the counts still do not |
| 2026-08-22 | **A tenth rendering style: `Rendered Illustration — No Visible Hand`** (user-directed, from a reference image they were trying to match). The gap it fills: their target was a high-finish digital painting with no visible mark, and the library had nothing for it — Production Painting is brushwork-supporting-form and Photo Real explicitly refuses illustration, so whichever they picked pushed away from the look. The user wrote the Image-Model Prompt; the Description, Operating Principle and nine Visual Mechanics are that paragraph split into the document's own sections, nothing invented beyond it. New `markRendered` plate diagram — the plate palette's own greys stepped as continuous tone, one sharp specular, one soft shape behind | Stage 02, Board Rendering picker (and every render prompt, via the Bible's Rendering Language) | Built from canon — the rich card and the plate vocabulary already existed, and the document is the only place the style is stated. Review (1) `No Visible Hand` beside Photo Real's `No Mark Of The Hand`, which are two subtitles a reader has to hold apart in one picker; (2) whether ten cards is past the point where the picker wants grouping — painted / drawn / technical — rather than one flat list |
| 2026-08-22 | **The palette adder is a button, not a live row** (user-hit: "I am still getting the odd Oxide color swatch that shows up when there are no other color swatches"). It was never a swatch: `.swatch-add` held an `<input type="color" value="#8a4b2e">`, which a browser draws as a filled rust square, sitting in the column of a production that had none — and `#8a4b2e` was a magic hex nobody chose, the same one seeding the swatch-proposal prompt's JSON example. `Add swatch` now opens the modal that already edited a swatch, so there is ONE statement of what a swatch is (name · colour · value-key pair) instead of two forms that had already drifted — the inline row could not state a pair at all, though the endpoint has always taken one | Stage 02, Color Palette column | Built from canon — an existing modal and an existing ghost button. Review (1) whether a colour input may ever sit unopened on a surface that also displays colours, since the two are visually the same object; (2) the dashed `.swatch-add` frame now holding a single button, which may want to be just the button |
| 2026-08-22 | **The Bible states when its medium disagrees with the anchor** (user-hit: Photo Real set, an oil painting back). The bible's `Rendering Language` section is now DERIVED from the rendering-style anchor and rebuilt whenever the anchor names a different style — so in the normal case there is nothing to show. What survives is the one case the app must not overrule: a director who edits that section by hand back onto a different medium. `#style-status` then carries a `REJECTED` badge reading `MEDIUM` beside the existing `REV n` badge, and one sentence naming both sides — "the Art Direction Bible's Rendering Language says Production Painting, and the board rendering style is Photo Real". The Model Test refuses on the same condition rather than spending a render proving it | Stage 02, step 4 Bible panel | Built from canon — an existing badge in an existing status line. Review (1) whether `REJECTED` is the right badge for a CONTRADICTION rather than a refusal — nothing here was rejected, two things disagree; (2) whether a status line is the right home for a condition that blocks a render two panels away, or whether it belongs beside the Model Test's own verb as a stated gate |
| 2026-08-22 | **Two controls stop speaking machine** (both user-directed). (a) The **board-type select** offered `LIGHTING_STUDY` — a machine token on screen, which the user had already ruled out the same day on `BOARD_LAYOUT_STYLE` ("that underscore is not for end users, has no meaning"). `BOARD_TYPES` has carried `label: "LIGHTING STUDY"` since it was written; the sheet editor rendered through it and the intake rendered around it. One list now, with each type's definition on it, composing BOTH selects' tooltips — the intake's named three of the five, and the intake is where a sheet is made. (b) The wizard's **Camera grammar** hint read "the production default — every panel inherits it unless it overrides", which describes the mechanism; it now reads "choose the default lens — you can override it when you render", which describes what the reader does | Stage 03 New breakdown + Blank sheet, stage 03 sheet editor, stage 02 Camera grammar | Built from canon — no new pattern, two labels and two tooltips. Review the copy only: whether a hint should state the MECHANISM or the READER'S NEXT ACT, since this system has both and they are not interchangeable |
| 2026-08-22 | **A gate you cannot act through** (`.pal-gate`, user: "it's not disabled, totally unclear, I can add swatches — disable this section with an obvious callout on why"): stage 02's Color Palette manual swatch adder is genuinely disabled — every input and its verb — while no Art Direction Bible is saved, behind an `--accent-line` framed, `--accent-soft` filled block with a Courier amber kicker `LOCKED — NO ART DIRECTION BIBLE` and one 12px sentence, the user's own copy verbatim: "Color swatches reference the Art Direction Bible once created. **Step 4**" — the step named in bold amber inline, with **no verb** (ruled 2026-08-22: the step is on the same page and a button that only scrolls is not worth a primary's weight). Everything in the column that STATES a palette locks together — the swatch adder, `In words` and `Add images` — and the block sits above the first of them; only the existing thumbs and their viewer stay reachable, because those only read. It is the column's only amber while locked, and it removes itself the moment a Bible exists | Stage 02, Color Palette column | New pattern: canon has stated gates (`.up-gate`, 9.5px `--ink-faint` beneath a disabled control) but nothing for a gate that must OUT-WEIGH the control it governs. The faint line was tried first and the user's reply was "totally unclear" — it is compliant and unreadable beside a live input. Review (1) whether `.up-gate` and `.pal-gate` should be one scale with two weights rather than two patterns; (2) amber-on-amber-soft as a block fill, which no other surface uses; (3) **the product question underneath**: a hand-added swatch is attached to the Bible draft as a reference photo, so gating it removes the director's ability to state palette colour BEFORE the Bible is written. The user asked for the gate knowing the column also holds `Add images` and `In words`, which stay live; if colour-in-advance matters, the gate belongs on generation only |
| 2026-08-21 | **Preview the read** (owner tools only): Settings &rarr; Debug tools gains a `Preview the read on stage 01` verb that replays the reading surface against the screenplay the install already holds. It walks the SAME local parse the real read does, so the ladder, the snapshots and the observations are real; what it cannot do is call a model, so it stops at a fourth phase, `previewed`, which states `PREVIEW ONLY — THE PARSE IS COMPLETE AND REAL. NO MODEL WAS CALLED, SO THE READ'S OWN FINDINGS ARE NOT HERE.` and offers `Close preview`. The header's engine slot reads `PREVIEW — NO MODEL CALLED` in place of an engine name | Settings &rarr; Debug tools; stage 01 | Built from canon — review the copy and the fourth phase only. The panel itself is the 2026-08-20 read surface. Worth a ruling on (1) whether a phase strip that stops short should show MODEL and FOUND at all, or drop them — today they sit unlit, which reads as "not yet" rather than "not coming"; (2) whether an owner-only preview belongs in the Uncanonized queue at all, since no customer can reach it |
| 2026-08-20 | **The read, as it happens** (user: "this part needs to be fun"): the screenplay's read takes minutes and reported itself with two toasts and nothing between them. A panel now leads stage 01 for the duration — a **scene ladder** (one Courier row per parsed slugline, its bar filling over the dwell), the scene's **own lines set as screenplay** beside it, and a ticker of observations that are counts (`VERA speaks in 3 of 5 scenes — probably a lead`). Three phases as a sentence, not a bar: PARSE (`--ok`), MODEL, FOUND. The panel's **one amber is the bar of the scene being read now** — the phase strip deliberately yields it, and during MODEL there is no amber at all, because nothing is current. Everything shown is parsed locally from `GET /api/screenplay/digest`; the model's phase says in words that it has one call and no per-scene progress to report, and is given no bar to lie with | Stage 01, while a read runs | New pattern: a **progressive** meter, which canon does not have — `.loc-meter` counts coverage in four fixed segments and is finished the moment it is drawn. Review (1) whether live feedback for an opaque remote call should be made of local work at all, or whether that is a borrowed progress bar wearing an honest label; (2) the ladder's dwell — it is paced to ~60s regardless of the real call, so a fast read finishes with bars still filling and a slow one sits in MODEL; (3) the screenplay-format snapshot, which is the first place the app sets the script as a script rather than as data; (4) whether FOUND should stay in this panel or hand off to stage 02 **Amended 2026-08-21 after the first watched run:** the panel is a FIXED height end to end — the snapshot column re-flowed per scene and the finding feed grew as it filled, so the surface popped for the whole read; both are reserved now and long content scrolls in place. The findings moved off the footnote and onto their own `--field` ground with an `--ok` rule, at 13px, newest at full ink and older ones stepping back. The MODEL phase gained the app's own `.busy` spinner reading `Scoping production needs`, and while it runs `body[data-read-busy]` disables every action and field in the view — navigation deliberately excluded, since locking someone out of their own app for the length of a model call is worse than the double-spend it prevents. |
...
| 2026-08-25 | **The production camera card is retired** (cinematography plan A4): the `Camera grammar` card inside the Cinematography anchor on stage 02 is removed — the row of five selects and its saved-status line. Consolidation put it there on 2026-08-16 because it could contradict the anchor beside it; that was the right diagnosis and the weaker cure, since a production-wide `Eye level · 24mm · Level · Wide` nobody chose still rode every prompt as a directive. The anchor's copy now says each grammar carries the framings it sanctions. `camera_defaults.json` survives as a silent fallback and the panel camera override is untouched | Stage 02, Cinematography anchor card | A REMOVAL — review whether the anchor card reads complete without it, and whether anything is now missing from stage 02 that a director expects to set there |
| 2026-08-25 | **Why this framing, and where it fights** (cinematography plan A3): a `.cam-note` under the framing select, empty most of the time. It carries the one line the breakdown pass wrote saying why that row and not its nearest neighbour, and — where the chosen row is not one the panel's grammar sanctions — a sentence naming the disagreement and the rows that grammar does sanction. A statement, never a gate: choosing a framing that pulls against a grammar is a director's decision. The select keeps showing a held row it does not sanction, labelled `— outside this grammar`, because dropping it would silently replace a deliberate choice on the next save | Stage 03 breakdown row, stage 04 workbench | Built from canon — `--ink-faint` prose, no amber. Review (1) whether a disagreement this soft is soft enough to be ignored entirely; (2) it is the first prose sentence inside a `.cam-field`, which forced a voice reset against the axis-label rule; (3) whether `— outside this grammar` in an option label is clear or reads as an error |
| 2026-08-25 | **The framing a panel renders at** (cinematography plan A2): a `Framing` select beside `Cinematography` in the camera row, on both the breakdown row and the panel workbench. Every option states its optics — `Intimate close-up — 75–100mm, f/2–2.8` — and the inherit option names the row it would inherit rather than saying "production default". Options are only the framings the panel's grammar sanctions, redrawn when the grammar changes. A matching `.shot-tag-framing` joins the take's badge stack, one step below the cinematography tag, stating focal length, aperture, focus and any modifier count | Stage 03 breakdown row, stage 04 workbench, take badge stack | Built from canon — the same `.cam-field` select as the five camera axes, the same right-hand badge stack. Review (1) whether a select is the right control for twenty options with two facts each, or whether this wants the rich style picker the grammars use; (2) the badge stack is now four tags deep in one corner; (3) `None — no framing` sits above the real options, matching the grammar select, but reads oddly when the list is all twenty |
| 2026-08-29 | **The cast screen** (PRODUCTION_DESIGN_UI_PLAN §3.4, revised): a roster grouped by kind under Courier kickers, six up, each card led by its picture at the subject's own ratio — `.8` for a character, `1.85` for a vehicle or prop, never letterboxed into the other — with name, subtitle and photo count beneath. The uncast sit under a hairline as dashed chips with the manual door inside the same block, because they have no picture and a list is the honest shape. Clicking a card opens a DETAIL SCREEN, not a modal: hero at the subject's ratio, an alternates filmstrip ending in ADD ANOTHER, then WHO THIS IS, WHAT RIDES EVERY PROMPT as rule-marked traits, and LIVES ON / RIDES AS in Courier | Stage 02 step 03 | Built from canon. Every door still goes through the one cast modal and the one photo chooser; the screen writes no card itself. Review (1) a detail with no photograph is a large hatched panel stating its blocker — right, or too big; (2) the roster and the Reference/Subjects shelf now show the same cards two ways; (3) six-up at 1420 gives a character card ~210px |
| 2026-08-29 | **The Bible reads as a document** (PRODUCTION_DESIGN_UI_PLAN §3.5, revised — the plan calls this "the ruling that matters most in this section"): three views over one file, behind a `READING | MARKDOWN` segment with the file path beside it. READING is the default and renders the markdown — `#` a 30px title, `##` a 19px Archivo heading over a `--hairline` rule in the document's OWN casing, `###` a Courier kicker, prose at 15.5/1.7 on a 68ch measure, an Avoid list as bordered chips, `**Design language:**` leads as grey inline labels, and a design language's own reference frame and swatch ramp beside it where the production has them. MARKDOWN is the literal file, read-only, under the parse warning. A third column carries the section index, WHAT WROTE IT anchor thumbnails, and the self-check | Stage 02 step 04 | Built from canon plus `.bible-doc` and `.bible-rail`. The renderer never writes, supports only what `draft_bible` emits, and escapes everything. Review (1) three columns at 1420 leaves the doc at 68ch with air either side; (2) whether MARKDOWN should be a view or a download; (3) the `##` under `## Design Languages` are language names and render at the same weight as real sections |
| 2026-08-31 | **The logline leads the stage** (user-directed): it leaves step 2's read strip for its own unnumbered `.wiz-logline` section above Style Anchors — intact, same markup, same amber rule, same 720px measure. A numbered step promises something is done to it there and nothing is; it is what the read understood the film to BE, and every step below answers to it | Stage 02, above step 1 | Built from canon — transliterated, not reinterpreted. Review (1) an unnumbered panel in a numbered column: does the rhythm hold; (2) whether PERIOD should follow it up rather than stay in the tile strip |
| 2026-09-01 | **The casting modal asks for the profile, and offers the second door** (user-caught): the profile field and the generate option were built on the cast DETAIL — the surface reached AFTER casting — so the one place a subject's words are first written had neither ("None of your changes seem to be in"). The modal now carries *Who this is* beside *Identity*, and a `.cast-gen` checkbox beside the photo tray that renders once the card exists, with the spend stated on the option. **Its type was also brought back to §1.3**: it had drifted to four sizes — 9, 10, 10.5 and 12.5 — three under the 11.5px floor, and the user could not read them. Now three: 15px title and closing verbs, 11.5px everything else | The casting modal | Built from canon. **The type fix is scoped to `.cast-modal` on purpose** — `.f-label` is 10.5px and `.hint` 12.5px APP-WIDE, which is the same drift on every other surface and deserves one deliberate pass rather than a side effect of a casting fix. Designer: is that pass worth doing? |
| 2026-08-31 | **A subject has a profile, and two doors to a picture** (user-directed, finishing §3.4's detail): a subject had ONE text field and the mocks show two sentences — the role on the roster card, the profile on the detail — so `description` is its own field now, shown as WHO THIS IS and edited by *Edit the description*. The filmstrip ends in the plan's dashed `.cd-more` **GENERATE ANOTHER**, and the acts row gains **Generate one** beside *Attach a photograph*: it renders from the subject's own words under the Bible and the approved anchors, at the subject's own ratio (4:5 character, 16:9 vehicle/prop — never letterboxed), and lands exactly where an upload lands, an APPROVED reference carrying the subject's role. The spend is stated beside the buttons, never discovered after. APPEARS IN states what would make it knowable rather than guessing — nothing in the app maps a subject to an act. The hero column went 560px→380px, the mock's proportion | The cast detail | Built from canon — `.ghost`, `.busy`, the same `--hold`/dashed vocabulary. Review (1) three ghost buttons in one row; (2) the header says N PHOTOS where the mock says N SCENES — scene counts per subject do not exist |
| 2026-08-31 | **The findings show what the read found** (PRODUCTION_DESIGN_UI_PLAN §3.3, the other half never built): design languages were Courier chips and are now image-led `.lang-card`s leading with a reference actually scoped to that language (`store.reference_language`), stating the blank when there is none; environments were a grid of cards stating a COUNT of locations and are now `.env-row`s NAMING those locations in Courier; and a new `.subj-rail` shows the subjects the read found as chips — dashed until cast, solid once carded — under **Cast what is uncast**, since the SUBJECTS counter previously pointed at a step without ever saying what it counted. NOT built: the plan's 120px thumb per environment row — an environment has no picture anywhere in this app and a slot that can never fill is worse than no slot | Stage 02 step 2, the findings | Built from canon. **Designer: where should an environment's picture come from?** That is the one part of §3.3 left open. Also review (1) four stated-blank language cards on a fresh production — is the hatching worth it before any reference is scoped; (2) `+ ADD DESIGN LANGUAGE` now sits in a row of 190px cards |
...
| 2026-09-14 | **Stage 01 loses its second route to stage 02** (user-directed: "remove this button"): `Review the read on Prod. Design` sat under the DOWNSTREAM OF THIS FILE counts. It was added 2026-08-31 to replace a button that removed itself after seven seconds — the TIMER was the fault and stays fixed; the button was the wrong repair. The numbered pipeline band is the navigation, it is on every view, and `02 PROD. DESIGN` is one click from that panel at all times. A second route under a block of counts competes with the band that already does it. The counts themselves are untouched | Stage 01, the downstream panel | Built from canon — a control removed. Review whether any OTHER stage carries a duplicate route the band already provides; this was the only one on 01 |
| 2026-09-14 | **The locations table stops shouting** (user-caught: "this doesnt look corrected. Is it?" — it was not): two things the 09-13 copy pass classified wrongly. `NEEDS THE BIBLE` sat on all seventeen rows, one fact restated per line beside a SHEET column already reading NONE — it is one sentence above the table now, with the route to step 04, and the cell is an em dash. And `TURNS ON — <23-word narrative sentence>` was `.toUpperCase()`d into Courier: I had kept it as "provenance", but provenance is a screenplay quote with a page number, and this is prose. It is Archivo in its own case now, with only the TURNS ON kicker left in Courier. The ENVIRONMENT column header dropped its `— ITS VISUAL RULES` | Stage 02, the locations table | Built from canon — two rules already written, applied where they had not been. Review whether the act group's `READ — 7 · SHOWING 5` and the `Expand — 2 more` row read clearly now that the block around them is quieter |
...
| 2026-09-12 | **The type floor is measured in CAP HEIGHT, and the Courier step is 15px** (user-caught: "site still has 9px tall fonts"): LEGIBILITY_FLOOR names its fault as "Courier labels rendering at 8–9px cap height" and prescribes a 13px font-size. Measured by pixel scan, Courier New's cap is 8px at 11.5, 13, 14 AND 15px — it is hinted flat — so implementing the plan exactly moved the rendered letter height by zero and left the fault it was written for. Both levers that reach cap height moved the minimum needed: `--mono` leads with Consolas / Cascadia Mono (Consolas is NARROWER than Courier New, so no label costs more room), and the Courier step is 15px, which renders the 10px cap Archivo gives at the 14px prose floor. Every Courier rule now carries a size, because one inheriting the 14px body rendered 9px caps | Every surface | **Needs a ruling on the FACE.** Rule 2 names Courier and this changes which mono actually renders — one line in `--mono` reverts it, at the cost of 8px caps. Also review (1) the 13px step is now unused, so the live scale is 14/15/16/17/19/20/24/34; (2) `.prod-grid` went to `minmax(0,1fr)` because 15px Courier pushed it 45px past a 1100px window |
| 2026-09-12 | **The colour floor is measured, not listed** (LEGIBILITY_FLOOR_DELTA_2026-09-12b): the first pass ported a LIST of greys; the delta rules that the implementation must port the RULE — every text colour measures ≥ 7:1 against the surface it sits on — and says "a lint that walks computed styles is the right shape". It is one now, and it found what a list could not: `--hold` (#7d8fd0, 5.70:1) colouring 29 glyphs and `--bad` (4.61:1) colouring 44. Both move to the RULE beside the words, which is where the delta already put red: a held or failed thing carries a 2px left rule or its own border and reads in `--ink-faint`. Ten marks whose state was ONLY a colour (`.stor-line.hold`, `.req-mark.hold`, `.gen-warn`, `.made-blocker`, the `.rail-mark` and `.prod-care` families) would otherwise have become three identical greys; they gained the rule instead | Every surface | RULED. Review (1) `--panel2` is the one ground where the plan's own inks land under 7:1 — 6.56 for `--ink-faint`, 6.96 for `--accent` — and fixing it means darkening a surface token the floor does not mention; (2) `--hold` now colours nothing anywhere, so "proposed" reads as a dashed border rather than a blue word; (3) a danger button's label is ink and its border carries the red |
...
| 2026-09-12 | **The roster counts scenes, and casting is one gesture** (CAST_CHARACTER_SCREEN §1): a card's third line is `n SCENES` rather than a photograph tally — what the draft says about a subject, not what the app holds. An uncast chip and the manual `+ Cast` row now CAST and land on the empty detail screen, where the modal used to ask again for a name, a role line and a profile the read already supplied. The plan's 12px floor is applied across both screens, including `#cast-screen`-scoped overrides for `.wiz-group-label`, `.cost` and `.text-act`, which are app-wide at 9.5–10.5px | The cast roster and detail | Built from canon. Review the scoped type overrides: they are a patch over an app-wide 9.5px drift that still wants a deliberate pass |
| 2026-09-12 | **The cast screens show their pictures for the first time** (bug fix, found from the DOM): `castShot()` returned `background-image:url("…")` and every caller interpolated it into a `style="…"` ATTRIBUTE, which ends at the first inner double quote — the browser parsed `style="background-image:url("` and discarded the URL. Every picture on the cast row, the roster and the detail had been missing since those surfaces were built, silently and all at once, which is why it read as *the pictures do not work* rather than as one wrong character | The cast row, roster and detail | Built from canon — no new pattern, the pictures simply arrive. Review nothing; noted because three surfaces change appearance |
| 2026-09-11 | **Casting carries what the read found** (user-caught: "the screenplay read should get all these character profiles — why is the character information not populated?"): the read returns a subtitle and a page of traits for every subject, and BOTH casting doors were rebuilding a bare name+kind from the tile's data attributes — so the modal opened blank and the card was cast with nothing on it. One `recFor()` now hands over the read's own record; a hand-typed name still falls back to an empty one. The read is additionally asked for a `description` — the look, as the sentence an art department hands an illustrator — which is the field WHO THIS IS shows and a generated reference renders from | The casting modal, from the step-03 ribbon and the cast screen | Built from canon — no visual change, a data-carrying fix. Review: the modal now opens with six trait lines pre-filled, which is a lot of text to arrive already written |
| 2026-09-11 | **The file opens; only the model reads** (user-directed): stage 01's `Read the screenplay` became **Open screenplay**, matching the two other doors to the same file. "Read" is this app's word for the model pass — the one that spends and produces the findings, and the thing the whole stage-01 surface is named after — so a free button that opens a PDF was taking the expensive verb | Stage 01, Current screenplay panel | Built from canon — copy only. Review whether `Open screenplay ↗` (the third door, which carries the arrow) should be the single form |
| 2026-09-11 | **A catalogue pick ends the proposal standing on the card** (user-caught: "selected cinematography style and said 'use this' and nothing changed"): two doors answer an anchor — accepting the proposal, and picking from the catalogue — and only the first cleared `.ah-prop`. A pick SAVED correctly and stayed invisible, because the overlay sits at z-index 3 and went on covering the card, still offering *Use this* for a different style. One `anchorAnswered(role)` now ends a proposal for all three doors (accept, dismiss, pick) | Stage 02 step 1 anchor cards | Built from canon — no visual change. Review: the picker's own confirm button is also labelled **Use this**, the same words as the proposal overlay's — two different acts wearing one verb is what made this read as the proposal failing |
| 2026-09-11 | **Accepting one proposal no longer discards the other two** (user-caught: "why did cinematography and Board Rendering get unset?"): they were never set — accepting the texture proposal made the arrival guard's `anySet` true, so the next render skipped the suggestion entirely and the other two proposals, already paid for and cached, stopped being offered. Their cards fell back to *Not set*, which reads as something taken away. The guard now runs while ANY anchor is still empty, and `showProposals` skips a card that is already answered — the same rule, applied per card instead of across all three | Stage 02 step 1, on arrival | Built from canon — no visual change, a gate grain fix. Review whether a card whose proposal was dismissed should stay dismissed across a reload (it currently re-offers from cache) |
| 2026-09-11 | **A chosen style reads SELECTED, and the period stops citing one screenplay** (user-caught): the anchor badge answered IN WORDS for anything that was not a picture, so accepting a proposal or picking from the catalogue labelled the card as if the words had been typed by hand — and `syncAnchorBadges`, which runs after the row and on every change, knew only NONE ↔ IN WORDS and overwrote the row's answer a tick later. One `anchorWords()` now answers for both: **SELECTED** when the words are a catalogue style, IN WORDS when they are the director's own, NONE when empty. Separately the PERIOD card's consequence line named a failure from one production's screenplay (a development note, not app copy) and treated the string "UNSTATED" as a value — it now uses `wizNoPeriod`, offers **State it**, and says the read found no period rather than claiming renders are held to one | Stage 02, step 1 anchor badges and the read strip's period card | Built from canon — no new tokens. Review (1) SELECTED vs IN WORDS as the two words for a words-only anchor; (2) whether the period card should read as needing attention when unset rather than neutral (a 2026-08-24 question, still open) |
| 2026-09-10 | **The generate gate reads before it is hit** (user-caught): pressing *Generate one* with no saved Bible spun a `.busy` with its elapsed clock running — the server had refused in milliseconds, but the handler called `stop?.()` on an object whose disposer is `.done()`, which threw inside the `finally` and swallowed both the error and the spinner. The two conditions the server refuses on are knowable on the card, so both generate controls are now DISABLED with the reason stated beneath them in a `.cd-blocked` `--hold` rule, carrying a link to step 04 where it is resolved. The cost line is replaced by the reason rather than shown beside it — a price for something you cannot buy is noise | The cast detail | Built from canon — `--hold` for not-ready, never `--bad`; the existing gate grammar. Review (1) a disabled primary-looking ghost button beside two live ones; (2) whether the same gate belongs on the casting modal's render checkbox, which states NEEDS A SAVED BIBLE but is not disabled |
| 2026-09-10 | **Step 03 is a row of twelve, and it does not scroll** (user-directed; REVERSES the drag-to-scroll ribbon of 2026-09-01 below): `.cast-ribbon` is a 12-column grid filling the width beside the button, so a tile went from 62×78 to 99×124 and is legible at a glance. What the twelve do not show is COUNTED on the act that reaches it — `Open the cast +12` — rather than hidden off-screen behind a drag with no affordance. Every tile still acts: an uncast one casts in place, a cast one opens its own card | Stage 02 step 3 | Built from canon. Review (1) a cast-but-unphotographed tile and an uncast one differ only by a dashed border and the word UNCAST at this size; (2) whether twelve should fall to fewer on a narrow window rather than shrinking |
| 2026-09-01 | **Step 03 is a working ribbon** (user-directed, correcting the 2026-08-31 row below): that row was a PREVIEW — three photographed thumbs, a rule, three dashed chips and a text link — so nothing in it did anything and every act still meant opening the roster. It is now one `.cast-ribbon` holding the WHOLE cast plus every uncast name: an uncast tile opens the casting modal in place, a cast tile opens that subject's own card (the detail, not the roster). It scrolls and it DRAGS (pointer events, `touch-action: pan-y`), with a 4px guard so a drag that ends on a tile does not also click it. **Open the cast is a `.ghost` button, not a text link** — a link read as a caption on a row of pictures, and opening the full cast is now optional rather than the only way in | Stage 02 step 3 | Built from canon. This REVERSES yesterday's B3 call that only photographed subjects lead the rail: a hatched tile under a name is not an empty shape when the click that fills it is right there. Review (1) 32 tiles at 62px on one row; (2) the drag has no visible affordance beyond the cursor |
| 2026-08-31 | **Step 03 is a row; the cast is a screen** (PRODUCTION_DESIGN_UI_PLAN §3.4, the half of that plan never built): the step rendered the whole roster inline at 1,546px with the uncast list appearing TWICE, under two separate manual-add rows. It is now one `.cast-row` — up to three cast thumbnails, a rule, three dashed uncast chips with the rest counted, and **Open the cast**. The roster and the single uncast list live on `#cast-screen`, shown by `body[data-cast="1"]` (which hides the steps, the logline and the stage head) rather than floating in a modal — every card on it is a card on Reference / Subjects, and looking at one is not a dialog. Bulk casting moved onto the surviving list so no way of casting was lost | Stage 02 step 3, and the cast screen | Built from canon — review placement/copy only. Review (1) the step now shows no picture at all until a subject has one (B3), so a fresh production sees chips and a count; (2) whether the screen should keep the stage's step rail for orientation |
| 2026-08-31 | **The stages are held while the model reads** (user-directed, and a REVERSAL of the 2026-08-21 ruling that navigation stays live): `data-read-busy="1"` now dims and disables `#nav` and `#tools-nav` as well as the view's own controls ("the app allows me to switch tabs while it is 'scoping production'. I shouldn't be able to switch until it is done"). Every downstream stage is about to be rewritten by the answer in flight, so a stage reached mid-read shows numbers with a shelf life of seconds. The read panel STATES the hold rather than leaving dead buttons to be discovered, and carries the way out: a `.busy-cancel` **Stop waiting** on the busy row — not in the ticker, which runs to hundreds of scene rows, because an escape under the fold is not an escape. It stops the app waiting, never the read: the answer saves server-side either way and still toasts when it lands | Stage 01, model phase of a read | Built from canon — `.busy-cancel` is the existing vocabulary, down to its "stops waiting for the result" phrasing. Review (1) a whole-app lock for a call that can run minutes; (2) whether Settings should stay reachable, since a failing key is resolved there |
| 2026-08-31 | **The read walk is 10s and uneven; its route is permanent** (user-directed): the scene ticker spread a finished local parse over 60s at an identical interval per scene ("each scene is the exact same length of time") — now 10s with ±45% jitter per row, clamped 160–700ms. The finish panel showed a count and a Prod. Design button for 7s then removed itself, putting a primary action on a timer; it now shows nothing and goes AT ONCE — the 7s hold was removed too (measured: 25ms to clear, and that is the stage repainting, not a wait) — and the route is a permanent full-width `.ds-go` ghost button in the stage-01 side rail under the counts the read produced | Stage 01, during and after a read | Built from canon — the button is `.ghost`, since stage 01's one primary act is uploading a draft. Review (1) the walk is honest content at a chosen pace — whether it should exist at all; (1b) the stage now arrives with no beat between the last scene row and the filled inventory; (2) the route's placement under DOWNSTREAM OF THIS FILE |
| 2026-08-31 | **Stage 01 arrives settled** (bug fix, user-reported): `renderScreenplay` painted the template, awaited three fetches, then removed the Replace panel — so arriving at the stage with no screenplay showed a full side rail and took half of it away again ("this section on the right flashes on then goes away. janky"). The fetches now happen first and the stage paints once; the previous view holds for the ~80ms, which is what every other tab already does. No new tokens or classes | Stage 01, on arrival | Built from canon — review placement/copy only. Review (1) the ~80ms where the previous view is still up, against the alternative of a skeleton; (2) a failed fetch now leaves the old view plus an error rather than a half-painted new one |
| 2026-08-30 | **A proposal shows the look it proposes** (bug fix, user-reported): the three PROPOSED anchor cards named a style in words over a flat `--bg` panel, so the one card whose whole job is "here is a look, do you want it" showed a blank rectangle. `.ah-prop` is now the card's only scrim — a `to top` gradient like `.ah-scrim`, a little stronger for four lines — with the card's own hidden under `.anchor-hero.proposed`; the proposed style's own picture fills the frame behind it, resolved through the same matcher a hand-pick uses. The state badge reads `PROPOSED` in `--hold` rather than amber, the vocabulary its own kicker already speaks. Also: rendering slot 0 is the HOUSE slot, whose key the client rewrites to `house` — the plate manifest still files that style's photographs under the document key, so the first rendering style was the one style that could never show a photograph anywhere | Stage 02 step 1, the anchor cards while a read's proposals stand | Built from canon — review placement/copy only. Review (1) the reason now sits over a picture rather than a flat panel — legibility at the top of the gradient; (2) `PROPOSED` appears twice on the card, as badge and as kicker |
| 2026-08-30 | **The anchor hero paints what was actually chosen** (bug fix, user-reported): the three style-anchor cards in stage 02 showed hatching over anchors that were correctly set and stored. Three faults, each sufficient — the row painted before the interview filled its fields, before the style catalogues loaded, and it looked only for photographed frames. A style whose plate is drawn rather than shot (5 of the 10 board renderings) now paints that diagram as a data URI, whole, on `--field` rather than cropped to `cover`. New: `.ah-shot.drawn`, and an `.anchor-hero::before` top scrim mirroring `.ah-scrim` — the role and state labels washed out over a bright sky the moment real pictures began arriving | Stage 02 step 1, the three anchor cards | Built from canon — review placement/copy only. Review (1) the drawn plate at 62% on `--field` beside two photographed frames: does the mixed row read as one set; (2) the top scrim's weight (.72 vs the bottom's .93) |
| 2026-08-31 | **Stage 01 waits for the read** (user-directed): while the read is running, every panel on the screenplay stage hides except the read itself — the inventory, the citations block and the whole side rail. They return when it settles, with a 220ms `.stage-in` fade. Built because every number on that stage is about to change, and a page of counts that hold for the next forty seconds is a page you must re-read afterwards to know which of it still stands. A FAILED read gives the stage back too — the inventory is still true and it is the only thing left to act on | Stage 01, during a read | Built from canon. The fade is the plan's 220ms for a layer opening and is NOT an entrance animation on load — §2.5 forbids those and this only fires on the transition out of a read; it honours `prefers-reduced-motion`. Review (1) the stage goes to a single panel on a black field for ~40s; (2) whether the side rail's file facts are worth keeping visible since they do not change |
| 2026-08-30 | **The screenplay read takes the screen** (user-directed): the anchor suggestion's button, cost line and ladder are gone from stage 02. The read runs on arrival and locks the page behind `.sp-takeover` — a full-viewport scrim at 75% `--bg` with a 2px blur (amended 2026-08-30: it was opaque, which read as the app having gone away rather than as a pause) carrying a spinner, `Processing screenplay`, and one Courier line `READING IT FOR THE LOOK · ONCE PER DRAFT`. It appears only after 400ms, so the cached answer never flashes it, and it clears with the proposals painted on the cards. No bar: the read is one model call with nothing to see inside | Stage 02, on arrival | Built from canon — the `.busy` spinner enlarged, and it honours `prefers-reduced-motion` the way that one does. Review (1) a full-screen lock for a read that is usually cached; (2) the takeover has no cancel, so a slow model holds the stage; (3) whether 'Processing screenplay' or 'Reading the screenplay' is the better word |
| 2026-08-29 | **The screenplay proposes the look, and colour leaves step 01** (user-directed): a `Suggest from the screenplay` ghost act above the anchors, stating `COSTS A MODEL CALL · PROPOSES, NEVER SETS`, whose result lands as an `.ah-prop` overlay on each hero — a `--hold` dashed panel carrying `PROPOSED FROM THE SCREENPLAY`, the style's name, the reason, and `Use this` / `Dismiss`. Accepting writes exactly what a manual pick writes. Colour is no longer a step-01 anchor: its palette is proposed FROM the Bible, so it now sits in step 04's aside under `COLOUR — PROPOSED FROM THE BIBLE, NOT CHOSEN BEFORE IT`, and step 01 reads `n OF 3 SET` across three columns | Stage 02 steps 01 and 04 | Built from canon — `--hold` is the PROPOSED vocabulary the design languages already use, and no amber is spent on a card that already carries an overlay. Review (1) whether a proposal should cover its card or sit beneath it; (2) three anchors leaves THE BOARDS as one column against THE MOVIE's two; (3) whether the palette wants its hero at all in a 320px aside |
| 2026-08-29 | **Production Design restandardised** (PRODUCTION_DESIGN_UI_PLAN_2026-08-28, appearance only): the stage title and a Courier rail replace the head's label; anchor cards become 1.586:1 image heroes with the role top-left, the state top-right in amber Courier and the chosen look's name and line over a scrim, the whole card being the control; the SETS/NOT jurisdiction moves off the cards into all four pickers; the palette gets its own modal since it has no catalogue; the model choice becomes a segmented picker taking an amber BORDER and never a fill, dimmed when locked or gated; every spending verb states its cost in Courier beside it; the Bible becomes two columns with `SECTION HEADINGS ARE PARSED — DO NOT RENAME` over the editor; the sample acts become text links; and the run ladder gains one binary stage bar per phase — grey, amber, green — with no percentage and nothing creeping | Stage 02, all five steps | Built from canon plus five new blocks (`.anchor-hero`, `.seg`, `.cost`, `.bible-cols`, `.rl-phase`). **Three of the plan's own instructions were NOT followed** and are ledgered with reasons: two renames the user made after the plan was written, and the camera grammar row both mocks show, which stays retired by user ruling 2026-08-29. Review (1) whether the four heroes read as looks at this size; (2) the anchor state is the only amber in step 01 — correct, or too quiet; (3) §3.3's findings order and §3.4's casting block are NOT yet done |
| 2026-08-28 | **The run ladder, and three named acts** (user-directed): stage 01 gains an `Inventory Script` button under a `Script Inventory` headline — its deterministic parse was an invisible act with no button and no name. Stage 02 step 2 becomes `Build Design Plan` (was `Script scene scan`, which named a mechanism and collided with stage 01), and the rail chip follows. Step 4's verb becomes `Build Art Direction Bible`, so the three acts share one grammar. Both new acts use `.rl`, a generalisation of the Bible draft's `.rd` live panel: phases across the top, then one row per real result — label in Archivo `--ink-faint`, value in Courier, `--ink` once it lands and an em dash in `--ink-faint` until it does. **No progress bar anywhere**, because where a phase is one model call there is nothing to measure. Stage 01's ladder stays after it finishes (the act takes under a second, so the ladder IS the inventory); the Bible's still dismisses | Stage 01 main panel, stage 02 step 2 | Built from canon — reuses `.rd`. Review (1) whether a sub-second act deserves a ladder at all or wants a different shape; (2) two ladders and the Bible's panel now sit on one pipeline, all slightly different; (3) `Inventory Script` vs `Script Inventory` — verb button, noun heading — is the pattern; step 2 uses the same words for both |
| 2026-08-26 | **A take says when its framing did NOT ride** and **the camera step states the camera the render will use**: a `.shot-tag-didnt` variant reading `FRAMING — NOT IN THIS PROMPT · <NAME> WOULD RIDE A RECOMPILE` in `--ink-faint`, and the closed camera summary now leads with the framing and its optics, listing only axes the panel itself set. The framing picker also opens to all twenty rows, the non-sanctioned ones under an `optgroup` labelled `Outside this grammar — a choice, not an error`. All three are the same fault: a surface stating an intention as if it were a fact | Stage 04 take badge and camera step, stage 03 breakdown row | Built from canon. Review (1) the badge stack is four tags deep and one is now a two-clause sentence; (2) whether an optgroup is the right way to offer a choice the app mildly advises against; (3) the closed camera line is now long — `Subjective / poetic character · 50–100mm · f/1.4–2.8 · selective focus · low — subjective / poetic (this panel)` |
| 2026-08-25 | **The two-run style probe** (cinematography plan C6): each engine's style sample renders twice from one prompt and shows both side by side at equal size in a `.wiz-pair` grid, with one faint line saying what differs between them is the engine's variance and not a decision it made. A sample from before the rule, or one whose second run failed, shows one image and says so. Both open in one lightbox captioned `RUN 1 OF 2`. Built because twenty renders were read as evidence about a cinematography grammar over two days while nobody had established what two identical runs look like | Stage 02, engine style samples | Built from canon — equal-size pair, no first-and-second hierarchy, because making one larger would make it the answer and neither is. Review (1) two images per engine across three engines is now six pictures on one screen; (2) whether the variance sentence should appear once for the row rather than under each card |
| 2026-08-25 | **What the engine will accept** (cinematography plan C9): the prompt preview's `MADE OF · N CHARACTERS` head gains `· OF N THIS ENGINE ACCEPTS` where an engine has a stated limit, and the custom-engine card states `PROMPTS UP TO N CHARACTERS` or `NO STATED PROMPT LIMIT`. Only user-added engines have one — the built-ins have never refused a prompt, and 132 takes from 289 to 21,179 characters all succeeded, so giving them a figure would invent the constraint that measurement disproved. Over the limit is refused before the render with both numbers | Stage 04 prompt preview, Settings → Engines & keys | Built from canon — Courier machine data in an existing head, no new component. Review (1) whether "no stated limit" belongs on the card or is noise; (2) whether the preview should warn as it approaches a limit rather than only stating it |
| 2026-08-25 | **The bible read against itself** (cinematography plan R2): a `Check it agrees with itself` ghost button under the bible editor, and a `.bc-row` report beneath it — the two section names in Courier joined by `✕`, each rule quoted in Archivo, and one faint line saying what a single frame cannot do while both hold. Headed `N CONFLICTS — ADVISORY, NOTHING WAS CHANGED`. A clean read says how many characters it read and that deliberate contrast between different subjects was not counted. The act sits under the document rather than in the act row, which stays one primary verb plus Regenerate | Stage 02, under the Art Direction Bible editor | Built from canon — `--line` rule, `--ink-dim` headings, `--ink` quotes, `--ink-faint` consequence, no amber and no `--bad` because a contradiction the director meant is still their call. Review (1) whether an advisory report with no action beside it is useful or just noise; (2) whether the section names should link into the document; (3) `✕` between two headings — clear, or does it read as a close control |
| 2026-08-25 | **What the selection withheld** (cinematography plan R1.2): a `.pc-with` line closing the `.prompt-comp` panel, separated by a `--line-soft` rule. It names how many lines of Character Presentation were left out and who they describe, and says the lines that govern any frame still ride. Where the selector cannot tell, it says so instead — "no saving here" is a result and an empty panel is not. Built because the measurement above says a block is 31% of the prompt, and this says which paragraphs the app decided not to send; a selector that drops canon silently is the failure being fixed | Stage 04, prompt preview | Built from canon — `--ink-faint` prose, `--ink-dim` lead-in, no amber and no `--bad`, because withholding a paragraph about someone not in the frame is the system working. Review (1) whether this belongs inside the composition panel or beside it; (2) whether the withheld LINES should be readable, not just their subjects |
| 2026-08-25 | **What a prompt is made of** (cinematography plan R1.5): a `.prompt-comp` block above the prompt editor — one row per compiled block, biggest first, each with a value-ground bar, its share, its name in Courier and its character count, under `MADE OF · N CHARACTERS`, capped at 74ch — it ran the full workbench width and put each bar a screen away from its label. Closed with one line saying why it matters: a block describing something the panel does not contain is not neutral, it competes with the ones that do. Built because the figure that drove two days of work was counted by hand from one panel; the first automated reading corrected it and found CHARACTER PRESENTATION at 31.5%, describing two characters not in the frame | Stage 04, prompt preview | Built from canon — `--line` bars, `--ink-dim` labels, no `--bad` and no amber, because a measurement is not a verdict and no block is wrong on its own. Review (1) whether a bar chart belongs in this app at all, which has none elsewhere; (2) top-eight truncation with no statement of what was dropped; (3) whether the share or the character count is the number a director reads |
| 2026-08-25 | **What a take was rendered from** (cinematography plan C4): a second `.shot-tag` above the cinematography one, reading `SENT — HAND-EDITED ↗` or `SENT — THE COMPILE ↗`, opening the exact text the image model received. A take keeps two prompts — the steps 01–04 compile as governance record, and `render_prompt` when an override rode — and no screen showed either; `Preview prompt` reads the panel's CURRENT compile, which for any older take describes a render that never happened. The overlay leads with what was SENT, states both character counts, and shows the compile beneath it labelled as not sent | Stage 04, staged take's tag row | Built from canon — same tag grammar, same corner, no new tokens. Review (1) two stacked tag-buttons in one corner, which is a density canon has not ruled on; (2) whether `SENT` is the right word for a compile that was, in fact, sent; (3) whether the tag should say the character count itself, since that one number is the fact that mattered |
| 2026-08-25 | **No colour reference** (user: "I cant see a way to remove the color swatch reference"): the panel's Palette menu gained a third answer beneath a rule — a `.pal-row` whose ramp is an empty hatched slot rather than a colour, because the row's meaning is the absence of one. The menu chose WHICH palette rode and ticking nothing meant the shelf's automatic pick, so there was no way to say none; the foot line changed from `NONE SELECTED = AUTO` to `NOTHING TICKED = AUTO` now that the two are different answers. Sent to the server as `-COLOR_PALETTE`, since omitting a palette is what triggers the automatic top-up | Stage 04 workbench, palette menu | Built from canon — existing `.pal-row` grammar, no new tokens. Review (1) whether a negative option belongs in a list of positives or wants its own control; (2) the hatched empty ramp as a way of drawing absence, which canon has no precedent for; (3) whether the other auto-attached anchors (world texture, cinematography style, board rendering) deserve the same refusal, since the suppression mechanism is general and only the palette exposes it |
| 2026-08-24 | **The cinematography a take rode, readable** (first user test follow-up): the take's `GRAMMAR — X` tag became `CINEMATOGRAPHY — X ↗` and is now a button. It opens the style's full entry — description, principle, mechanics, avoid, films, and the block SENT TO THE MODEL VERBATIM — read live from `docs/CINEMATOGRAPHY_STYLES.md`, never copied, and footed with that path so the reader knows where to edit. Step 03's summary also names the panel's cinematography now; without it a saved change left the panel looking identical | Stage 04, the staged take's tag row | Built from canon — same tag, same corner, font and border inherited so it still reads as a fact rather than a control. Review (1) whether a tag that is also a button needs any affordance beyond the ↗; (2) whether the other tags (model, hash, size) deserve the same treatment, since every one of them names something a director might want to read; (3) `promptOverlay` was built for prompts and is doing duty as a document reader |
| 2026-08-24 | **A frozen step says why, on screen** (first user test follow-up): every frozen step in the panel workbench gained `.step-frozen`, a dim line under the head naming the take that froze it, the door out (Withdraw approval, on that take in the strip) and what is still allowed (generating is never blocked). Before, a frozen step showed only a green `✓ SETTLED` chip — which reads as GOOD — beside a disabled verb whose explanation lived in a hover title: "it says I cant change camera... I get the 'no' icon when i hover over change camera." Third instance of this same defect in a week, so the fix is in `seqStep` and covers every step rather than the camera alone | Stage 04 workbench, all sequence steps | Built from canon — ink, not `--bad`, because a settled panel is not an error. Review (1) whether `✓ SETTLED` is the right chip at all when it sits above a sentence explaining a refusal; (2) whether the line belongs under the head or beside the disabled verb it explains; (3) whether a one-take camera override should exist, the way the prompt editor already allows a one-take prompt — an approval settles the SHEET, and the user's instinct was that it should not settle the experiment |
| 2026-08-24 | **The app answers its own fields** (first user test, plan S2.2/S2.3/S2.5, and a re-request): Sample location became a real `<select>` of the screenplay's own locations with a `Somewhere else — type it…` escape — it was a `datalist`, which shows nothing until you type, so a user whose screenplay had just been read saw an empty box. Spec ID's note changed from `A NAME ONLY` to `NAMED FROM YOUR SCENE · EDIT IF YOU LIKE` (it already filled itself; the copy invited typing, and on the call it was dictated by hand). Step 5's heading is now `Test your model`. And the per-panel cinematography axis was relabelled `Grammar` → `Cinematography`, because the user who asked for it in August asked again in September, unable to find it | Stage 02 model test; stage 03 auto door; stage 04 camera editor | Built from canon — no new pattern, three of four are words. Review (1) whether a picker with a type-your-own option should be the house pattern for every field the scan can answer; (2) whether `Cinematography` beside `Angle · View · Lens · Tilt · Shot` reads as a camera axis when it is a grammar; (3) the manual breakdown door still says `A NAME ONLY` and keeps its example placeholder — deliberate, since there is no scene to name it from, but the two doors now differ |
| 2026-08-24 | **A pick you made outranks a pick you rendered** (first user test, plan D2.1/D3.2): the reference rows' reason text gained two states — `YOU PICKED THIS` / `YOU LEFT THIS OFF` — which replace `RODE THE PREVIOUS TAKE` once a panel has chosen for itself. The selection is now written to the panel rather than living in the DOM, because a take-derived memory can only remember what has already been RENDERED and the moment between ticking a reference and generating with it is exactly when a user has an intention. Beneath the attachment manifest, `.attached-note` states an attachment the panel will NOT get — a design language with no palette of its own — in ink, not `--bad`, because it is a fact about the render and not a fault | Stage 04 workbench, references step | Built from canon — existing row grammar and one dim note line. Review (1) whether two memories should be visible as two (a picked row and a rode-the-last-take row read identically apart from their reason text); (2) whether an un-rendered pick deserves a mark of its own, since it is the one state the take record cannot show; (3) the note's placement under the manifest rather than beside the palette group it concerns |
| 2026-08-24 | **Edit a reference** (first user test, plan D2.4): an `Edit` `text-act` on every reference card that reopens the role dialog prefilled with the reference's current family, title, and both jurisdiction lists. A tester filed a spaceship as `CHARACTER_LIKENESS` because that was the role the walkthrough named, and nothing in the app could take it back — the PATCH endpoint existed and no screen had ever called it, so the role picked at intake was permanent. Re-scoping keeps the approval and the image; only what the reference GOVERNS changes, and it is journaled | Reference library, every card | Built from canon — the existing role dialog, no new pattern. Review (1) whether `Edit` belongs beside Crop/Reject or under a quieter affordance, since it is rare and consequential; (2) whether the jurisdiction fields should be prefilled from the OLD family (as now, so nothing is silently lost) or reseeded from the new family's suggestions; (3) whether the card should show that a reference has been re-scoped |
| 2026-08-24 | **The production's period** (first user test, plan D1.4): a `.read-period` card beside the logline in stage 02's read strip — a Courier value (`241 YEARS IN THE FUTURE` / `UNSTATED`), a `text-act` to state or edit it, and one line of `.mini` saying what it costs to leave blank. Pre-filled by the screenplay scan, editable by hand so a production built before the field exists can state it without paying for another read. Every render is then held to it as a forbidden clause. His screenplay is set 241 years ahead and no panel ever said so, which is how a WW2 aircraft reached a far-future salt pan | Stage 02, read strip | Built from canon — Courier for the value because it is a constraint renders are held to, Archivo for the explanation. Review (1) whether it belongs beside the LOGLINE (both are what the read understood) or with the gates it enforces; (2) whether `UNSTATED` should read as a gap needing attention or as a neutral default — it is currently neutral, and it silently constrains nothing; (3) whether one line of consequence copy under a field is a pattern we want to repeat |
| 2026-08-23 | **An answer you can save** (first user test, retest): the interview answer field had no save affordance — Enter or lose it — so typing and clicking away discarded the answer with no request and therefore no feedback. **The row's own `Answer` button becomes `Save`** (`ghost`→`primary`) while the field is open, and `Decide later` becomes `Cancel`; `.q-edit` holds only the input and a Courier `ENTER SAVES · ESC DISCARDS` hint. A second Save button beside the first was tried and rejected by the user — 'the answer button should become save, not spawn a new button'. Clicking away SAVES; Escape is the only discard. The stated gate from the row below also grew from dim caption text into `.repair-gate`, a bordered full-ink chip — it was stated and still missed among eight toolbar controls | Stage 02 open questions; Repair Region dialog | Built from canon — `primary`/`text-act`/`mono`, no new tokens. Review (1) whether blur-saves is the right default for this app, since canon has no ruling on implicit commit; (2) whether a four-element edit row belongs inline or should displace the question text; (3) the chip's border weight against the disabled primary next to it |
| 2026-08-23 | **A refusal that states its condition** (first user test, plan B2): two controls refused silently in the first forty minutes. Repair Region's primary is disabled until the user has BOTH painted and typed an instruction, and said neither — a `.mini mono` gate line now sits left of the button naming whichever condition is unmet, and reads before the first stroke rather than after it. The locked-stage popover gained `.bp-tally`, a dimmer second line under a gate's label carrying its live count (`3 TAKES RENDERED · NONE APPROVED YET · ONE IS ENOUGH`) — the author guessed the wrong rule about his own gate live on the call, so the label alone was not enough | Repair Region dialog; stage-band lock popover | Built from canon — `.mini mono` and `--ink-faint`, no new tokens, never amber (a gate is a statement of fact, not the live signal). Review (1) whether a gate line belongs LEFT of its button or beneath it — canon has no placement rule for a condition attached to one control; (2) whether the tally's two-line step breaks the popover's rhythm at four steps; (3) the voice — these are Courier state strings, but they read as sentences |
| 2026-08-23 | **Publish to every studio** (user: "it is important that we set this up so an admin's changes are migrated to the apps at next redeploy"): the Alt-click text editor's drawer gained a third `.ghost` act beside Clear. Text overrides are now two layers — a SHIPPED layer in source that rides every deploy and reaches every studio and every downloadable copy, and the existing LOCAL layer which is this install's scratchpad and wins on conflict so an editor always sees their own words. Publish promotes local into source where the repo is writable, and where it is not — every cloud tenant — it writes nothing and returns the exact JSON to commit, in the `promptOverlay`. It never reports success for an edit that reached one volume | Debug text-edit drawer | Built from canon — review placement/copy only. Review (1) whether a two-outcome act belongs on one button or wants a stated precondition beside it the way canon's gates do, since on a tenant it can only ever refuse and the drawer does not say so before it is pressed; (2) whether "every studio" is the user's word for the fleet; (3) whether the returned-JSON case should be an overlay at all, or a copyable block in the drawer |
...
- **2026-08-23** — Text overrides split into a shipped layer (`app/content/ui_text.json`, rides every deploy) and the existing local layer (this install only, wins on conflict); `Publish to every studio` in the debug drawer promotes one into the other, and states plainly when it cannot. The READ route is open — published copy must render on a studio whose owner has no debug tools.
Newest first. One line per change, dated. Amend the relevant section above as
well — this log records what changed, it does not replace the rules.

- **2026-08-22** — **The breakdown intake goes back to two doors** (user instruction, reversing RULE_PASS_2 C). The blank door is no longer a superset — no brief, no paste, no panel list — so the two are genuinely different acts on different endpoints, and amber moves to the stronger one.
- **2026-08-22** — **Bible + palette are one watched act.** New `.bb-ramp` and `.sw-collide`; the read panel's `.rd-*` vocabulary carries a second long operation. Diagnosed the same day: four design languages shared one palette because the bible's section spec never asked a language for a colour of its own.
- **2026-08-22** — **The Bible is created and saved by one button.** `Create Art Direction Bible` writes and saves; the editor below is read-only until `Edit`, which becomes `Save`. Also: the gate chain's `ADD STYLE REFERENCE` became `DESCRIBE THE LOOK` and now counts an anchor stated in words, not only an approved picture.
- **2026-08-22** — **World texture's picker plates are the user's own renders**, three scenes per texture (the same three subjects under all five conditions, so the set reads as one comparison). The `PLATES ARE DIAGRAMS` apology now shows only over an actual diagram — it keyed off "has a plate", which stays true once a library gains photographs, so two libraries were apologising for diagrams they no longer showed.
- **2026-08-22** — **The cinematography grammar becomes a per-panel axis.** `Grammar` joins the camera row with inherit / None / a named grammar; the picker now says it sets a default that any render can change. Also: the eight grammars' picker plates are the user's own renders, three scenes each.
- **2026-08-23** — **Open questions say they are optional** — `OPTIONAL — YOU CAN DO THIS OVER TIME` in `--ok` beside the count.
- **2026-08-23** — **Palettes approve as one.** Per-colour Approve retired; Edit/Reject stay per colour, the verdict is the group's. Anchor badge counts palettes (`4 PROPOSED`, never 24, never NONE over pending colour); proposal rows carry Edit/Approve on the row.
- **2026-08-22** — **A plate row is as wide as it has frames**, and the captured house plate opens full size (it was the one picture in the picker with no `data-lb`). New `.rs-frames[data-n]`; `[data-lb] { cursor: zoom-in }`.
- **2026-08-22** — **Five rendering styles get real plates.** The picker is the first partly-photographed library: five frames in, five diagrams still standing.
- **2026-08-22** — **Rendering styles: nine become ten.** `Rendered Illustration — No Visible Hand` fills the gap between Production Painting's visible mark and Photo Real's refusal of illustration. New `markRendered` plate diagram.
- **2026-08-22** — **The palette adder stops looking like a swatch.** Its pre-filled colour input read as an unexplained rust swatch in an empty column; the form moved into the modal that already edits one. Also: the anchor/Bible reconciliation now covers world texture and cinematography, not just the rendering style.
- **2026-08-22** — **The rendering anchor is upstream of the Bible.** Changing the board rendering style rewrites the Bible's Rendering Language section from the style's own document entry — deterministic, free, no button (a production set Photo Real, ran the Model Test and got an oil painting, because the section still said Production Painting). A hand-edited section that disagrees is STATED on the Bible panel and refused by the Model Test, never overwritten.
- **2026-08-22** — **Board types read as English, and say what they do.** `LIGHTING STUDY` replaces `LIGHTING_STUDY` on screen and one list composes both selects' definitions, where the intake's tooltip had documented three of five. The Camera grammar hint states the reader's next act rather than the inheritance rule.
- **2026-08-22** — **The selection gets its own row.** Only the scene's title lands in `What should I get?`; the brief, the line pointer and `Clear selection` take the paste row's slot, and the paste row returns when the selection is dropped. The results list gains `None` and `Paste a section` at its head — it is the door's source switch now, because a selection hides the paste row. New: `.door-row[hidden]`, `.scene-hit.is-act`.
- **2026-08-22** — **A picked scene is a pointer, not a guess.** The breakdown search auto-fills on typing again (the submit-only version from the same morning is withdrawn), and choosing a scene saves the line its slugline sits on. `Scan Screenplay` reads THAT scene: `insights.scene_anchor_at` quotes it exactly and refuses if the draft changed under the pointer, where `scene_anchor` had to work out which scene the words meant and got it wrong twice.
- **2026-08-22** — **The palette adder is gated on the Bible.** New `.pal-gate` block (UNCANONIZED): disabled controls, amber framed callout, the reason and the door. Replaces a `.up-gate` line the user called "totally unclear". Also: twelve stale wizard step numbers corrected — the 2026-08-16 renumbering shifted the panels and left the prose behind, and `tests/test_step_numbers.py` now derives the numbering from the markup.
- **2026-08-21** — **The read panel is stage 01's, and it leaves when the work does.** It had been mounting into whichever view was open, so it followed the user to stage 02; and its finished state sat on the stage indefinitely. It now mounts only into `.dash-main`, and the FOUND state clears itself after a beat with the result carried by the toast. The Scene Scan reports through its own busy line, as it always did.
- **2026-08-21** — **The read stops moving while it is watched, and says what it is doing.** Fixed panel height (both columns and the finding feed), findings promoted onto their own ground, a `Scoping production needs` spinner for the model phase, and a page-wide action lock for its duration. Also: screenplay grammar (INT, EXT, DAY, CONTINUOUS…) can no longer be reported as a finding about a draft.
- **2026-08-21** — **A read can be previewed without an engine.** Owner-only, on the Debug tools tab; adds a fourth phase `previewed` to the read panel rather than letting a preview wear the MODEL phase's copy. No new CSS.
- **2026-08-20** — **Stage 01 reports the read while it runs.** New `.rd-*` block, marked UNCANONIZED, tokens only. The one amber is the current scene's bar; the phase strip uses `--ink`/`--ok` so the panel never carries two. Logged in the table above — which is now at FIVE rows, past the review threshold.
...
- **2026-08-17** — **Non-canon: citations are verified at the write.** An
  ...
  skipping the only falsifiable one. Three write paths now verify; the visible
  consequence is rows arriving at HOLD, and two new counts with nowhere to go
  yet — hence the table row rather than a silent fix.

- **2026-08-17** — **`.mini` prose stops wearing Courier's clothes** (user: "the
  description font is a strange choice. check against design system"). `.grid-form
  label, .fgroup` puts LABEL typography on the container; controls and `.hint`
  reset it but `.mini` reset only the FAMILY, so helper prose inherited
  uppercase + 700 + .13em and rendered as Archivo shouting — against §2 (Courier
  carries machine data, Archivo carries prose) and §Copy (sentence case in prose).
  `.mini` now carries the full prose reset. **And the scan is reframed**: it takes
  free direction, not a query, and proposes the brief as well as the content.

- **2026-08-17** — **Scan screenplay moves to where you look at panels** (user:
  "Dont see it" — it had shipped only on the breakdown editor, ~3600px down).
  It now leads step 02 REQUIRED on the workbench, and an accepted find is
  written server-side with the screenplay line as SCRIPT_EXPLICIT evidence.
  Objects join the brief and the camera as things amendable between takes.

- **2026-08-16** — **Non-canon: Scan screenplay on a panel.** Building it found a
  live bug in `scene_anchor`: it stopped at the FIRST location overlapping the
  subject, so a board in "TERRA NOVA SECURE BAY" anchored to "TERRA NOVA" and
  the composition check had been judging framing against the wrong scene.

- **2026-08-16** — **The breakdown editor stops over-locking**, plus two smaller
  user fixes: the plate viewer's Close becomes **Save** once you change the
...
- **2026-08-16** — **Non-canon: an object can say a plate is not its reference.**
  ...
  though the object is an airlock. A matcher cannot read grammar, so it has to
  be overrulable per object.

- **2026-08-16** — **Non-canon: engines declare whether reference images reach
  the image model.** Two of three do; the ChatGPT pipeline cannot, and every take
  of the confrontation board had gone through it — plates attached, manifest
  saying seven references, image model receiving none. The rewriter also now
  DESCRIBES what it sees in a plate instead of pointing at it.
...
- **2026-09-14** — RETIRED: `.ds-go` and the `go-wizard` button — the pipeline band is the route to stage 02.
- **2026-09-14** — Locations: the Bible gate is stated once above the table (`.loc-gate-one`) instead of on all seventeen rows; `.loc-turn` becomes Archivo in its own case, Courier kicker only; the ENVIRONMENT header loses its definition.
...
- **2026-09-12** — TYPE FLOOR measured in cap height: `--mono` leads with Consolas; the Courier step is 15px (13px retired); every Courier rule carries a size. Smallest rendered cap height app-wide goes 8px → 10px, measured at 1100 and 1560px.
...
- **2026-09-10** — `.cast-ribbon` becomes a 12-column grid (the drag-scroll and `.dragging` are retired); `.cast-open i` carries the count of what the row does not show.
- **2026-09-01** — `.cast-gen` (a stated-cost render option on the casting modal); `.cast-kind` and `.read-mark` raised to the 11.5px floor, and `.cast-modal` scoped to three sizes.
- **2026-08-31** — `.cd-more` becomes the dashed GENERATE ANOTHER slot; `.cd-spend` (a stated render cost) and `.cd-none` (a fact that is not knowable yet saying what would make it so).
- **2026-08-31** — `.lang-cards` / `.lang-card` / `.lang-shot` / `.lang-body` (design languages as image-led cards), `.env-rows` / `.env-row` / `.env-locs` (environments as rows, replacing `.env-grid` / `.env-card`), `.subj-rail` / `.subj-chip`.
- **2026-08-31** — `.cast-row` and its parts (`-thumbs`, `-shot`, `-rule`, `-chip`, `-more`, `-gap`), `.cast-head`, and `body[data-cast="1"]` — the cast screen replacing the stage.
- **2026-08-31** — `.wiz-logline` (the logline's own unnumbered section above the stage) and `.ds-go` (a full-width ghost route in a side rail).
- **2026-08-30** — `.ah-prop` changed from an opaque `--bg` panel to a bottom gradient so the proposed style's picture reads behind it; `.anchor-hero.proposed` hides the card's own `.ah-scrim` so only one scrim ever stands; `.ah-state.prop` (a proposal's badge, in `--hold`).
- **2026-08-30** — `.ah-shot.drawn` (a drawn style plate shown whole rather than cropped) and `.anchor-hero::before` (a top scrim for the role/state labels, the mirror of `.ah-scrim`). `.sp-takeover` changed from opaque `--bg` to 75% with a 2px blur — the stage stays visible behind the lock.
