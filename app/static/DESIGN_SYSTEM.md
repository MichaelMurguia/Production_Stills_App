...
| 2026-08-29 | **NOT YET BUILT — the plan's §3.3 and §3.4**: the findings order (logline at 20px, design languages as image-led cards with Confirm/Drop, environments as 120px-thumb rows, subjects as chips in a right rail) and the casting block (uncast chips under one dashed bordered block, manual add inside it on the same modal, cast cards 6-up stating `KIND · n SCENES`, photo modal naming the downstream reference type). Carried here rather than lost with the deleted plan file | Stage 02 steps 02 and 03 | Not a pattern to review — an open item. It needs building before the stage reads as one design |
