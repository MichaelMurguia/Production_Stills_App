...
- [ ] **Part C** — stages 03/04/05, film rolls, ledger, stale-tab bar
      (10 rows; mostly ratification, small diffs — do this next)
- [ ] **Part B** — the anchor cards (9 rows; the real work: one component
      with two lives, shared render, `Add your own` currently ships in two
      places and must collapse to one). Mocks: `rp-4a`, `rp-4b`
- [ ] **Part D** — Productions as a tab; the self-reporting act (2 rows)
- [ ] **Part E** — store queue, against `STORE_DESIGN_SYSTEM.md` (2 rows)
- [ ] **B10 blocker** — a take records whether the cinematography grammar
...
- [ ] Delete each `RULE_PASS_*` file as its part lands, ledger it in
