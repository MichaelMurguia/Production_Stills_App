...
| `camera_recipes.py` | The framing a panel renders at: one of the twenty rows in `docs/CAMERA_RECIPES.md`, resolved panel-choice → grammar's default → none, plus modifier deltas, the FRAMING lines that lead the CAMERA block, and the stamp each take records. Exists because a grammar written in adjectives (`selective focus`, `negative space`) is satisfied by an everything-sharp frame; `50–100mm at f/1.4–2.8` is not. Rows themselves come from `style_docs` |
...
| `wizard.py` | Scene Scan (schema-forced JSON), re-run merge semantics (confirmed survives by name; new arrives PROPOSED), faction self-check, bible drafting (`_bible_instructions` — DIRECTOR'S ANSWERS ARE BINDING). Also reads the Bible against itself (`bible_self_check`) for two rules that cannot both hold in one frame — advisory, never edits, and told by name that designed contrast between different subjects is not a fault |
...
| `activity.py` | Append-only `data/activity_log.jsonl` per production. Secrets redacted by FIELD NAME (key/token/secret/b64/image_url) **and by VALUE** — every credential the install holds, matched literally, plus a shape match for a key being verified before it is stored. The value pass was added 2026-08-23: the middleware logs `str(e)` and 4xx/5xx response bodies under `error`, which matched no name marker, so an upstream that echoed the credential wrote it into a log that rides a backup |
| `secrets_at_rest.py` | Wraps the credentials in `settings.json` so the file alone is not enough (2026-08-23 audit). `dpapi` on Windows (CryptProtectData, bound to the user account, stdlib ctypes); `key` on a cloud studio (AES-GCM under `SCREENBOARD_SECRET_KEY`, a per-tenant Railway variable the provisioner sets and the volume never holds); `none` elsewhere — and `status()` says which, because a wrap we cannot perform must never be reported as one. Applied at the single choke point, `generate.load_settings`/`save_settings`; callers only ever see plain strings. An unreadable value reads as ABSENT, never as ciphertext handed to a provider. It is not, and does not claim to be, a defence against the operator of a hosted studio |
