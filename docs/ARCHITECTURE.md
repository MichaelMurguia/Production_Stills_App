...
| `insights.py` | Derived truth: `blocking()` (HOLD/GAP/**KEY**/SIZE/CITE + CARE advisories; a row may carry a `stage` hint naming the stage it blocks when `action` does not imply one — the KEY row's action is `settings`, which is no stage), `stage_summary()` (band + gate chain data incl. `scan_done`, `interview_answered`), slugline `locations()` map, keyword derivation, citation re-check, friendly activity feed |
