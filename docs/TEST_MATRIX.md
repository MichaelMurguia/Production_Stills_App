...
**2647 tests** across 137 files.


## Product app — 2426 tests in 120 files
...
| `test_a_subject_has_a_profile_and_two_doors.py` | 35 | A subject carries a profile, and two ways to get a picture. |
...
| `test_cast_is_a_row_and_a_screen.py` | 20 | Step 03 is a row on the stage, and the cast is a screen. |
...
| `test_production_design_ui.py` | 24 | PRODUCTION_DESIGN_UI_PLAN_2026-08-28 — the standing contracts. |
...
| `test_read_progress.py` | 45 | The read, as it happens. |
...
| `test_stage_one_needs_no_engine.py` | 18 | Uploading a screenplay costs nothing, so it does not wait for a key. |
...
| `test_suggested_anchors.py` | 27 | The screenplay proposes the look, and colour leaves step 01. |
